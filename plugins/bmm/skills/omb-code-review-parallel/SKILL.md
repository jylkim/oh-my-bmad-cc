...
After cleanup, read the story file to determine story status and the highest-severity unresolved issue. Then use **AskUserQuestion** to present next steps based on the outcome.

### Pass — Story status is `done`

**If TEA is available** (`/omb-tea-testarch-test-review-parallel` skill exists):

**Question:** "Code Review passed — all issues resolved. Proceed to Test Review?"

**Options:**
1. **Proceed to Test Review** — Run `/omb-tea-testarch-test-review-parallel`
2. **Continue reviewing** — Stay in current context for further review
3. **Done for now** — End pipeline

**If TEA is unavailable:**

**Question:** "Code Review passed — all issues resolved. Proceed to Simplify & Commit?"
...
### Rework — Story status is `in-progress`

Determine rework scope from the highest-severity unresolved issue classification.

**SEVERE** (acceptance criteria issues, TEA available):

**Question:** "SEVERE issues found — acceptance criteria need revision. Rework from ATDD?"

**Options:**
1. **Rework from ATDD** — Run `/omb-tea-testarch-atdd-parallel`
2. **Rework from Dev Story** — Run `/omb-dev-story-parallel`
3. **Continue reviewing** — Stay in current context for further review

**MODERATE** (logic/API changes, TEA available):

**Question:** "MODERATE issues found — logic changes and test updates needed. Rework from Dev Story?"

**Options:**
1. **Rework from Dev Story (with Test Automation)** — Run `/omb-dev-story-parallel`
2. **Continue reviewing** — Stay in current context for further review
3. **Done for now** — End pipeline

**MINOR** (style, naming, refactoring) **or TEA unavailable:**

**Question:** "MINOR issues found — code-only fixes needed. Rework from Dev Story?"

**Options:**
1. **Rework from Dev Story (code only)** — Run `/omb-dev-story-parallel`
2. **Continue reviewing** — Stay in current context for further review
3. **Done for now** — End pipeline
